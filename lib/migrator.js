import { customSplit, migrationSchema, modifiers, types } from "./util";

/**
 * Statement breakpoints to get table
 * @typedef {object} StatementBreakpoints
 */

export const statementBreakpoints = {
  "CREATE TABLE": 2,
...
export const generateMigrations = (sql) => {
  let blocks = [];
  const statements = /CREATE TABLE IF NOT EXISTS|CREATE TABLE|DROP TABLE|ALTER TABLE|TRUNCATE TABLE|INSERT INTO/gi;
  ...
  while (match != null) {
    ...
    if (operation !== "CREATE TABLE" && operation !== "CREATE TABLE IF NOT EXISTS") {
    ...
    if ((altermatched = alterstatements.exec(nextoperation))) {
      ...
      table = splitted[statementBreakpoints[operation]].replace(/`/g, "");
    }
...
export const getmigration = (table, schema, type) => {
  ...
  const migrationType = type[0].toLowerCase() !== "create" ? "table" : "create";
  const migration = schema.map(makeMigration).filter(m => m.length > 0);
  return {
      name,
      migrationToCopy: migrationSchema(migrationType, table, migration, "\t\t\t"),
      migration: migrationSchema(migrationType, table, migration, "\t\t\t\t\t\t"),
  };
}
